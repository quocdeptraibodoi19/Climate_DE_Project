...
sudo echo "SPARK_LOCAL_IP=${terraform -chdir=./Terraform output -raw spark_master_private_ip}" >> spark-env.sh
sudo echo "SPARK_MASTER_HOST=${terraform -chdir=./Terraform output -raw spark_master_public_dns}" >> spark-env.sh
sudo echo "SPARK_WORKER_HOST=${terraform -chdir=./Terraform output -raw spark_master_public_dns}" >> spark-env.sh
cd ..
