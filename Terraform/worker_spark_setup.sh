...
sudo su
sudo echo "export SPARK_LOCAL_IP=$$(terraform -chdir=./Terraform output -raw spark_worker_private_ip)" >> spark-env.sh
sudo echo "export SPARK_WORKER_HOST=$$(terraform -chdir=./Terraform output -raw spark_worker_public_dns)" >> spark-env.sh
cd ..
