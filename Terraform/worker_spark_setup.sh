...
sudo su
sudo echo "export SPARK_LOCAL_IP=${output.spark_worker_private_ip}" >> spark-env.sh
sudo echo "export SPARK_WORKER_HOST=${output.spark_worker_public_dns}" >> spark-env.sh
cd ..
echo "spark://${output.park_master_public_dns}:7077" | ./sbin/start-worker.sh
