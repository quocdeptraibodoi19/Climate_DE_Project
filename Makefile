...
cloud-spark:
	terraform -chdir=./Terraform output -raw private_key > private_key.pem && chmod 600 private_key.pem && ssh -o "IdentitiesOnly yes" -i "private_key.pem" ubuntu@$$(terraform -chdir=./Terraform output -raw spark_master_public_dns) -N -f -L 8083:$$(terraform -chdir=./Terraform output -raw spark_master_public_dns):7077 && rm private_key.pem
