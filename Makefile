...
cloud-airflow:
	terraform -chdir=./Terraform output -raw private_key > private_key.pem && chmod 600 private_key.pem && ssh -o "IdentitiesOnly yes" -i "private_key.pem" ubuntu@$$(terraform -chdir=./Terraform output -raw airflow_public_dns) -N -f -L 8082:$$(terraform -chdir=./Terraform output -raw airflow_public_dns):8081 && rm private_key.pem
